...
let ``TryParse with standard winning hand`` () =
    ...
    let result = TryParse (handArray, [])
    
    Assert.Single(result) |> ignore
    let (ParsedHand (kantsu, shuntsu, kotsu, toitsu)) = result.Head
...
let ``TryParse with one kantsu`` () =
    ...
    let result = TryParse (handArray, kantsu)
    
    Assert.Single(result) |> ignore
    let (ParsedHand (k, shuntsu, kotsu, toitsu)) = result.Head
...
let ``TryParse with Ryanpeikou shape returns 3 valid parses`` () =
    ...
    let toitsuList = result |> List.map (fun (ParsedHand (_, _, _, t)) -> t)
...
let ``TryParse with 3-4-4-3 shape returns 4 parses`` () =
    ...
    let toitsuList = result |> List.map (fun (ParsedHand (_, _, _, t)) -> t) |> List.distinct
