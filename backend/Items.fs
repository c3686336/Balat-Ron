...
let allItems : Item list = [
  ...
  { id = Guid.NewGuid(); name = "Pinfu";
    ...
    rarity = Common;
    cost = 100;
    effect = fun state _ event -> match event with | OnYakuCalc (p, m, t, _) when pinfup p m t -> [ItemEffect.ExtraScore (1, 10)] | _ -> [];
    state = Nothing };
  ...
  { id = Guid.NewGuid(); name = "Iipeikou";
    ...
    rarity = Common;
    cost = 100;
    effect = fun state _ event -> match event with | OnYakuCalc (p, m, t, _) when iipeikoup p m t -> [ItemEffect.ExtraScore (1, 0)] | _ -> [];
    state = Nothing };
  ...
  { id = Guid.NewGuid(); name = "Ryanpeikou";
    ...
    rarity = Uncommon;
    cost = 125;
    effect = fun state _ event -> match event with | OnYakuCalc (p, m, t, _) when ryanpeikoup p m t -> [ItemEffect.ExtraScore (3, 0)] | _ -> [];
    state = Nothing};
  ...
  { id = Guid.NewGuid(); name = "Ittsu";
    ...
    rarity = Uncommon;
    cost = 125;
    effect = fun state _ event -> match event with | OnYakuCalc (p, m, t, _) when ittsup p m t -> [ItemEffect.ExtraScore (2, 0)] | _ -> [];
    state = Nothing };
...
//     { id = Guid.NewGuid(); name = "Toitoihou"; description = "Grants +2 Yaku (score multipliers) if your hand consists entirely of four triplets (or quads) and a pair, with no sequences."; rarity = Common; cost = 50; effect = fun state event -> match event with | OnYakuCalc (p, m, t) when toitoihoup p m t -> [ItemEffect.ExtraScore (2, 0)] | _ -> [] }
  ...
  { id = Guid.NewGuid(); name = "Chitoitsu";
    ...
    rarity = Uncommon;
    cost = 150;
    effect = fun state _ event -> match event with | OnYakuCalc (p, m, t, _) when chitoitsup p m t -> [ItemEffect.ExtraScore (2, 0)] | _ -> [];
    state = Nothing };
